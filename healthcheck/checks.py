...
class LiteLicenceUpdateTaskHealthCheck(BaseHealthCheckBackend):
    def check_status(self):
        dt = timezone.now() - datetime.timedelta(seconds=settings.LITE_LICENCE_DATA_POLL_INTERVAL)
        if not Task.objects.filter(queue=LICENCE_DATA_TASK_QUEUE, run_at__lte=dt).exists():
            raise HealthCheckException("Lite Licence Update Task is not responsive.")
